...
@section('content')
    <div class="card">
        <div class="card-header">
            <h5>{{ __('admin.edit_product') }}: {{ $product->title_en }}</h5>
        </div>
        <div class="card-body">
            <form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" class="form-control @error('title_en') is-invalid @enderror" id="title_en" name="title_en" value="{{ old('title_en', $product->title_en) }}" required>
                        @error('title_en')
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" class="form-control @error('title_ar') is-invalid @enderror" id="title_ar" name="title_ar" value="{{ old('title_ar', $product->title_ar) }}" required>
                        @error('title_ar')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <textarea class="form-control @error('description_en') is-invalid @enderror" id="description_en" name="description_en" rows="4">{{ old('description_en', $product->description_en) }}</textarea>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <textarea class="form-control @error('description_ar') is-invalid @enderror" id="description_ar" name="description_ar" rows="4">{{ old('description_ar', $product->description_ar) }}</textarea>
                ...
                <div class="mb-3">
                    <label for="primary_image" class="form-label">{{ __('admin.primary_image') }}</label>
                    @if($product->primary_image)
                        <div class="mb-2">
                            <img src="{{ asset('storage/' . $product->primary_image) }}" alt="{{ $product->title_en }}" style="max-width: 200px; max-height: 200px;">
                        </div>
                    @endif
                    ...
                    @error('primary_image')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                    <div class="form-text">{{ __('admin.leave_empty_keep_current') }}</div>
                </div>
                ...
                <div class="mb-3">
                    ...
                    @error('other_images.*')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                    <div class="form-text">{{ __('admin.upload_replace_existing') }}</div>
                </div>
